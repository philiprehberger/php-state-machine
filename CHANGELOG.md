...
### Added
- State entry/exit hooks via `onEnter()` and `onExit()` on the builder
- `StateMachine::rollback()` for reverting the most recent transition
- `StateMachine::toMermaid()` for generating Mermaid state diagrams
